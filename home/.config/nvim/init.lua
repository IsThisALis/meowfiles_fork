...
require("lazy").setup({
	...
	--
	-- 		-- Find and patch the existing buffer source to use all buffers
	-- 		local cmp = require("cmp")
	-- 		for _, source in ipairs(cmp.core.sources) do
	-- 			if source.name == "buffer" then
	-- 				source.source.get_bufnrs = function()
	-- 					return vim.api.nvim_list_bufs()
	-- 				end
	-- 			end
	-- 		end
	...
	},
	{
		...
		config = function()
			require("auto-save").setup({
				trigger_events = { "InsertLeave", "TextChanged" }, -- save after leaving insert mode or editing in normal mode
				...
				write_all_buffers = true, -- set to true if you want to save all modified buffers at once
				debounce_delay = 500, -- in ms, useful for TextChanged to avoid saving too often
			})
		end,
	},
